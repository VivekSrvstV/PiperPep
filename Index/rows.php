...
<?php //		echo "<table class =\"tab\" border=\"-1\" width=\"1200px\"  height='2%' name=\"tab\" STYLE=\"border-spacing: 0px;border-style: dashed ;
//		border-color:pink;
...
//echo "<center><tr bgcolor=\"#FF9900\"><th>VIRsiRNAid</th><th>Sense Sequence</th><th width=130px><a href=\"browse.php?by=".$_GET['by']."&TYPE=".$_GET['TYPE']."&sort=name of virus&how=".$how."\"><img src=\"images/Up-and-down-arrow.jpg\" height=\"15\"><font color=\"black\">Virus_Name</font></a></th><th><a href=\"browse.php?by=".$_GET['by']."&TYPE=".$_GET['TYPE']."&sort=Target Gene&how=".$how."\"><font color=\"black\"><img src=\"images/Up-and-down-arrow.jpg\" height=\"15\">Target</font></a></th><th><a href=\"browse.php?by=".$_GET['by']."&TYPE=".$_GET['TYPE']."&sort=Cell Line&how=".$how."\"><font color=\"black\"><img src=\"images/Up-and-down-arrow.jpg\" height=\"15\">Cell_Line</font></a></th><th><a href=\"browse.php?by=".$_GET['by']."&TYPE=".$_GET['TYPE']."&sort=object&how=".$how."\"><font color=\"black\"><img src=\"images/Up-and-down-arrow.jpg\" height=\"15\">Target_Object</font></a></th><th><a href=\"browse.php?by=".$_GET['by']."&TYPE=".$_GET['TYPE']."&sort=object&how=".$how."\"><font color=\"black\"><img src=\"images/Up-and-down-arrow.jpg\" height=\"15\">Efficacy</font></a></th><th>Align_With</th><th>Offtarget</th><th> PID</th><th>Family conserve</th><th>Virus conserve</th></tr></font></center>";
...
if(file_exists("./images/virus/$row[0].jpeg")){
  $family_con="siRNA sequence matching with \"<a href>$row[1]</a>\"  <a>$row[5]</a>\" Virus reference Genome sequences<img src=\"images/virus/$row[0].jpeg\" width=300 height=250>";}
else{$family_con='Sorry unable to load image';}
...
$var1="Pubmed:<a>$row[20]</a> <br/>Article:<a>$file_ex1[1]</a><br/>Authors:<a>$file_ex1[2]</a><br/>Journal:<a>$file_ex1[3]</a><br/>Entrez:<a>$file_ex1[5]</a><br/>";}
}
...
echo "<center><tr ><td><center>$font<a href=\"record.php?details=".$row[0]."\"rel=".$row[0]."0>".$row[0]."</a></td><td
style=word-break:break-all;
><left>".$row[1]."</td><td><center>$row[2]</a></td><td><center>".$row[3]."</center></td><td><center>".$row[4]."</center></td><td><center><a href=\"browse.php?by=$row[6]&TYPE=Target\" rel=".$row[6]."5>".$row[6]."</a></center></td><td><center>$row[11]</center></td>";

if (preg_match("/US/", "$row[8]")) {

echo"<td><center><a href=\"http://www.patentlens.net/patentlens/patents.html?patnums=".$row[8]."\" rel=".$row[0].">".$row[8]."</center></a></td>";}

else {echo"<td><center><a href=\"http://www.ncbi.nlm.nih.gov/pubmed/".$row[8]."\" rel=".$row[0].">".$row[8]."</center></td>";}
#<td><center>".$row[8]."</center></td>";

#<td><center>$HUMAN <a href=\"siRNAspecificity.php?ID=$row[0]\"><img src=\"images/$sss_color\"> </a><a href=\"siRNAseed.php?ID=$row[0]\">SL<a></center></td><td><a href=\"blast.php?SEQUENCE=$row[1]&VIRUS=$row[5]&id=$row[0]\" rel=".$row[0]."1>refseqs</a></td><td>&nbsp&nbsp&nbsp&nbsp&nbsp<a href=\"conserve_stats.php?CON=virus&CID=$row[0]\"rel=".$row[0].'fami><img src="images/virus/small/'.$row[0].'.jpeg"'.'></a></td>';
...
echo "<div id=\"$row[0]0\" class=\"balloonstyle\" style=\"width: 250px; background-color: #D0F5A9\">$browse_record</div>";
